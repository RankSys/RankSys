...
public class CachedUserNeighborhood<U> extends UserNeighborhood<U> {
     ...
     */
    public CachedUserNeighborhood(FastUserIndex<U> uIndex, Stream<Tuple2<U, Stream<IdDouble<U>>>> neighborhoods) {
        super(uIndex, new CachedNeighborhood(uIndex.numUsers(), neighborhoods.map(un -> new IdxObject<>(uIndex.user2uidx(un.v1), un.v2.map(vs -> new IdxDouble(uIndex.user2uidx(vs.id), vs.v))))));
    }
}
